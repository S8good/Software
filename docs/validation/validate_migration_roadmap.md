...
3. **CI Integration (planned)**
   - When CI is configured, add a job executing:
     ```bash
...
- 2025-11-03：已落地 `--strict` / `--report-file` / `--history-file` 参数，并补充 `tests/test_validate_migration.py` 覆盖严格模式、报告输出与历史记录；CI 自动化仍待后续配置。

## Mid-Term Enhancements
- 持续积累 `docs/reports/validation_history.csv` 指标，并使用 `scripts/plot_validation_trends.py` 生成趋势图，为运维提供可视化参考。
- CI 配置完成后，增加历史指标图表或仪表盘展示，以便运维快速评估迁移质量。
