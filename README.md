...
本迭代版本进一步强化了数据库工作流：处理方法可保存为不可变版本，
光谱保存时可记录结构化质量检查结果，数据库中的光谱可被重新分析并
以新的分析运行追加保存，原始数据和历史分析不会被覆盖。
...
分析菜单中的 `LSPR AI Workbench...` 现在以论文配对参考流程为主，当前支持：

- 分析物注册和选择
- 同一 chip/site 的 BSA 参考谱与反应谱配对
- 波长网格、数值和身份校验
- CEA 论文模型适配器接口
- 其余九种分析物的模型未提供状态和数据归档
- 旧版通用单谱结果的兼容读取

论文模型需要配对参考谱、反应谱、一阶导数和 BSA 参考特征。当前仓库不包含论文训练数据或模型制品；没有配置兼容的 CEA 模型时，软件不会生成数值预测，也不会回退到通用单谱后端。
